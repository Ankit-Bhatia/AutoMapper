...
| **🟡 KAN-90 — Entity relationship graph** | In progress on branch `codex/KAN-90-relationship-graph`: adds `relationshipGraph.ts`, wires `RelationshipGraph` + scoped `relationships` onto `AgentContext`, and passes target-entity scope into the optimizer so lookup targets can be rejected when `referenceTo` points outside the loaded target schema. | MEDIUM |
