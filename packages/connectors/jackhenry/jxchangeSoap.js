...
function normalizeDataType(input) {
    ...
    if (v.includes('date') && v.includes('time'))
        return 'datetime';
    if (v.includes('date'))
        return 'date';
    if (v.includes('time'))
    ...
    if (v.includes('int'))
        return 'number';
    if (v.includes('decimal') || v.includes('money') || v.includes('amount') || v.includes('numeric'))
        return 'decimal';
    if (v.includes('code'))
        return 'string';
    if (v.includes('name'))
...
function toFieldCandidate(node, sourceService) {
    const keys = Object.keys(node);
    const lowered = new Map(keys.map((k) => [stripNs(k).toLowerCase(), k]));
    const nameKey = lowered.get('elemname') ??
    ...
    if (rawName.length > 200)
        return null;
    const pathKey = lowered.get('xpath') ??
        lowered.get('path') ??
        lowered.get('elempath') ??
        lowered.get('msgpath');
    const typeKey = lowered.get('datatype') ??
        lowered.get('type') ??
        lowered.get('elemtype') ??
        lowered.get('fldtype');
    const jxchangeXPath = pathKey ? textOf(node[pathKey]) : undefined;
    const fieldName = rawName.includes('.') ? rawName.split('.').pop() ?? rawName : rawName;
    if (!/^[A-Za-z_][A-Za-z0-9_]{1,120}$/.test(fieldName))
        return null;
    return {
        name: fieldName,
...
export function buildSvcDictSrchEnvelope(credentials, serviceName) {
...
export async function postServiceGatewaySoap(creds, envelopeXml, soapAction) {
...
export function parseSvcDictSrchFields(xml, sourceService) {
    ...
    const responseRoot = findFirstByTag(body, 'SvcDictSrchResponse') ??
...
export function toServiceGatewayCredentials(credentials) {
    const c = credentials;
...
export function mergeConnectorFields(baseFields, liveFields) {
    if (liveFields.length === 0)
        return baseFields;
    const byName = new Map(liveFields.map((f) => [f.name.toLowerCase(), f]));
    return baseFields.map((field) => {
        ...
        return {
            ...
            jxchangeXPath: typeof field.jxchangeXPath === 'string' && field.jxchangeXPath
                ? field.jxchangeXPath
                : live.jxchangeXPath,
        };
    });
}
export function objectToSvcName(objectName) {
    switch (objectName) {
        case 'CIF':
            return 'CustInq';
        case 'DDA':
