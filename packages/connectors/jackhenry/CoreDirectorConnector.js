...
import { buildSvcDictSrchEnvelope, mergeConnectorFields, objectToSvcName, parseSvcDictSrchFields, postServiceGatewaySoap, toServiceGatewayCredentials, } from './jxchangeSoap.js';
export class CoreDirectorConnector {
    mode = 'mock';
    credentials = {};
    accessToken = null;
    async connect(credentials) {
        const creds = {
            authMode: credentials?.authMode === 'service-gateway' || credentials?.authMode === 'oauth-client-credentials'
        ...
        const hasServiceGatewayCreds = Boolean(creds.endpoint &&
            creds.username &&
            creds.password &&
            creds.instRtId &&
            creds.instEnv &&
        ...
        try {
            const selectedAuthMode = creds.authMode ??
    ...
    async fetchSchema(objectNames) {
        ...
        const fields = [];
        const relationships = [];
        const liveSvcDictCache = new Map();
                ...
                if (svcName) {
                    try {
                        ...
                        if (liveFields.length > 0) {
                            schema = {
                                ...schema,
                                fields: mergeConnectorFields(schema.fields, liveFields),
                            };
                        }
                    }
                    catch {
...
function buildCoreDirectorSchema(objectName, _entityId) {
    const id = () => uuidv4();
    const schemas = {
        CIF: {
            label: 'Customer Information File',
            description: 'Master record for each customer relationship. ISO 20022: Party. ' +
        ...
        DDA: {
            ...
            description: 'Checking and savings accounts. ISO 20022: CashAccount. ' +
        ...
        LoanAccount: {
            label: 'Loan Account',
            description: 'Consumer and commercial loans. ISO 20022: Loan. ' +
        ...
        GLAccount: {
            ...
            description: 'Chart of accounts entries. ISO 20022: Account. ' +
...
function buildCoreDirectorSamples(objectName, limit) {
    const samples = {
        CIF: [
