function firstNonEmpty(credentials, keys) {
...
function normalizeJackHenryCredentials(credentials) {
    ...
    if (instRtId)
        normalized.instRtId = instRtId;
    if (instEnv)
        normalized.instEnv = instEnv;
    if (validConsmName)
        normalized.validConsmName = validConsmName;
    if (validConsmProd)
        normalized.validConsmProd = validConsmProd;
    const hasOAuthCreds = Boolean(firstNonEmpty(normalized, ['instanceUrl']) &&
        firstNonEmpty(normalized, ['clientId']) &&
        firstNonEmpty(normalized, ['clientSecret']));
    const hasServiceGatewayCreds = Boolean(endpoint && username && password && instRtId && instEnv && validConsmName && validConsmProd);
...
export function normalizeConnectorCredentials(connectorId, credentials) {
