...
// ─── CoreDirectorConnector (mock mode) ────────────────────────────────────────

describe('CoreDirectorConnector — mock schema validation', () => {
  let connector: CoreDirectorConnector;

  beforeEach(async () => {
    connector = new CoreDirectorConnector(); // no credentials → mock mode
    await connector.connect();
  });

  it('returns the expected core entities and numeric code fields', async () => {
    const schema = await connector.fetchSchema();
    const names = schema.entities.map((entity) => entity.name).sort();
    expect(names).toEqual(['CIF', 'DDA', 'GLAccount', 'LoanAccount']);

    const entityByName = new Map(schema.entities.map((entity) => [entity.name, entity]));
    const fieldsByEntityName = (entityName: string) =>
      schema.fields.filter((field) => field.entityId === entityByName.get(entityName)?.id);

    // CoreDirector mock in this repo currently exposes descriptive type enums
    // (AccountType/LoanType) rather than numeric AcctType code values.
    const ddaAccountType = fieldsByEntityName('DDA').find((field) => field.name === 'AccountType');
    expect(ddaAccountType?.picklistValues).toEqual(['Checking', 'Savings', 'MoneyMarket', 'CDAccount']);

    const loanType = fieldsByEntityName('LoanAccount').find((field) => field.name === 'LoanType');
    expect(loanType?.picklistValues).toContain('LineOfCredit');

    const customerType = fieldsByEntityName('CIF').find((field) => field.name === 'CustomerType');
    expect(customerType?.picklistValues).toEqual(['Indv', 'Bus', 'Trust', 'Govt']);
  });

  it('propagates compliance tags on critical fields', async () => {
    const schema = await connector.fetchSchema();
    const entityByName = new Map(schema.entities.map((entity) => [entity.name, entity]));
    const fieldsByEntityName = (entityName: string) =>
      schema.fields.filter((field) => field.entityId === entityByName.get(entityName)?.id);

    const cifTaxId = fieldsByEntityName('CIF').find((field) => field.name === 'TaxID');
    expect(cifTaxId?.complianceTags).toContain('GLBA_NPI');
    expect(cifTaxId?.complianceTags).toContain('BSA_AML');
    expect(cifTaxId?.complianceTags).toContain('FFIEC_AUDIT');

    const ddaCurrentBalance = fieldsByEntityName('DDA').find((field) => field.name === 'CurrentBalance');
    expect(ddaCurrentBalance?.complianceTags).toContain('SOX_FINANCIAL');

    const loanCifNumber = fieldsByEntityName('LoanAccount').find((field) => field.name === 'CIFNumber');
    expect(loanCifNumber?.complianceTags).toContain('FFIEC_AUDIT');
  });
});
...
describe('SymitarConnector — mock schema validation', () => {
  let connector: SymitarConnector;
  ...
  it('contains the required CU object model and expected field/tag coverage', async () => {
    const schema = await connector.fetchSchema();
    const names = schema.entities.map((entity) => entity.name);

    // The connector currently exposes an extended CU model beyond the base 4 objects.
    expect(names).toEqual(expect.arrayContaining(['Member', 'Share', 'Loan', 'Card']));
    expect(names).toEqual(expect.arrayContaining(['IRS', 'Collateral']));

    const entityByName = new Map(schema.entities.map((entity) => [entity.name, entity]));
    const fieldsByEntityName = (entityName: string) =>
      schema.fields.filter((field) => field.entityId === entityByName.get(entityName)?.id);

    const cardId = fieldsByEntityName('Card').find((field) => field.name === 'CardID');
    expect(cardId?.complianceTags).toContain('PCI_CARD');

    const dividendRate = fieldsByEntityName('Share').find((field) => field.name === 'DividendRate');
    expect(dividendRate).toBeDefined();

    const memberNumber = fieldsByEntityName('Member').find((field) => field.name === 'MemberNumber');
