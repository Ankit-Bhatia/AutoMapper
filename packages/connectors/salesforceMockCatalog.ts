...
import type { RecordType as ConnectorRecordType } from './types.js';
...
interface MockObjectBundle {
  fields: Array<Omit<ConnectorField, 'id' | 'entityId'>>;
  recordTypes?: Array<Omit<ConnectorRecordType, 'id' | 'entityId'>>;
}
...
const CUSTOM_MOCK_OBJECT_BUNDLES: Record<string, MockObjectBundle> = {
...
const CUSTOM_MOCK_RECORD_TYPES: Record<string, Array<Omit<ConnectorRecordType, 'id' | 'entityId'>>> = {
  Account: [
...
export function getSalesforceMockObjectTemplates(
  ...
  for (const name of objectNames) {
    ...
    if (customFields?.length) {
      templates[name] = customFields as Array<Omit<Field, 'id' | 'entityId'>>;
      continue;
    }
...
export function getSalesforceMockObjectTemplatesForConnector(
  ...
  return Object.fromEntries(
    objectNames.map((name) => {
      ...
      return [name, baseFields as Array<Omit<ConnectorField, 'id' | 'entityId'>>];
    }),
  );
}

export function getSalesforceMockRecordTypeTemplates(
  objectNames: string[],
): Record<string, Array<Omit<ConnectorRecordType, 'id' | 'entityId'>>> {
  return Object.fromEntries(
    objectNames
      .map((name) => [name, CUSTOM_MOCK_RECORD_TYPES[name] ?? []] as const)
