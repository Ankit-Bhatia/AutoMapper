...
export function normalizeSalesforceType(input) {
    return SF_TO_INTERNAL[input.toLowerCase()] ?? 'unknown';
}
export function normalizeODataType(input) {
    return ODATA_TO_INTERNAL[input] ?? 'unknown';
}
export function typeCompatibilityScore(source, target) {
    if (source === target)
        return 1;
    const groups = [
